...
public class TeleOpBaseTest extends XKCommandOpmode {
    ...
    public TeleOpBaseTest(double targetX, double targetY, double startDeg, Pose2D hpZonePose, double headingOffset) {
    ...
    // false = 手动选择 shooter 配置；true = 根据坐标自动切换 close/mid/far
    private boolean autoShooterMode = false;
    private Shooter.ShooterConfig lastAppliedAutoConfig = null;
    ...
    public void run() {
        ...
        if (autoShooterMode) {
            double x = pinpointDriverData.getRobotX();
            Shooter.ShooterConfig targetConfig;
            String modeName;
            if (x > 15) {
                targetConfig = Shooter.shooterNearTop;
                modeName = "close";
            } else if (x > -20) {
                targetConfig = Shooter.shooterFar;
                modeName = "mid";
            } else {
                targetConfig = Shooter.shooterUltraFar;
                modeName = "far";
            }
            if (targetConfig != lastAppliedAutoConfig) {
                shooter.setShooterConfig(targetConfig).schedule();
                lastAppliedAutoConfig = targetConfig;
            }
            this.multipleTelemetry.addData("auto shooter mode", modeName);
        } else {
            lastAppliedAutoConfig = null;
        }
        this.multipleTelemetry.addData("shooter control", autoShooterMode ? "AUTO" : "MANUAL");
    ...
    public void functionalButtons() {
        ...
        */
        ...
        // 手动模式专用：Y / A / B / X 设置 shooter 配置；自动模式下被屏蔽
        new ButtonEx(
                ()-> !autoShooterMode && gamepad1.getButton(GamepadKeys.Button.Y)
        ).whenPressed(
                shooter.setShooterConfig(Shooter.shooter40cm)
        );

        new ButtonEx(
                ()-> !autoShooterMode && gamepad1.getButton(GamepadKeys.Button.A)
        ).whenPressed(
                shooter.setShooterConfig(Shooter.shooterNearTop)
        );

        new ButtonEx(
                ()-> !autoShooterMode && gamepad1.getButton(GamepadKeys.Button.B)
        ).whenPressed(
                shooter.setShooterConfig(Shooter.shooterFar)
        );

        new ButtonEx(
                ()-> !autoShooterMode && gamepad1.getButton(GamepadKeys.Button.X)
        ).whenPressed(
        ...
        new ButtonEx(
                () -> gamepad1.getButton(GamepadKeys.Button.BACK)
        ).whenPressed(
                () -> {
                    autoShooterMode = !autoShooterMode;
                    lastAppliedAutoConfig = null;
                    gamepad1.gamepad.rumble(150);
                }
        );
    }
}
